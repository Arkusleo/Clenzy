...
class ApiService {
  ...
  Future<List<Map<String, dynamic>>> getUserBookings() async {
     final headers = await _getAuthHeaders();
     final response = await http.get(Uri.parse('$apiUrl/bookings/customer'), headers: headers);
  ...
  Future<void> updateBookingStatus(String bookingId, String status) async {
     final headers = await _getAuthHeaders();
     await http.put(Uri.parse('$apiUrl/bookings/$bookingId/status?new_status=$status'), headers: headers);
  }
  ...
  Future<List<Map<String, dynamic>>> getPartnerJobs() async {
     final headers = await _getAuthHeaders();
     final response = await http.get(Uri.parse('$apiUrl/bookings/worker'), headers: headers);
     if (response.statusCode == 200) {
       return jsonDecode(response.body).cast<Map<String, dynamic>>();
     }
     return [];
